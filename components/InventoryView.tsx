            ...
            <div className="flex-1 overflow-auto">
                <table className="w-full text-left border-collapse table-fixed">
                    <thead className="bg-mist-300 sticky top-0 z-10 border-b border-mist-300 shadow-[0_1px_0_rgba(0,0,0,0.05)]">
                        <tr className="h-8">
                            <th className="px-4 text-[10px] font-bold text-slate-700 uppercase w-12 text-center">#</th>
                            <th className="px-4 text-[10px] font-bold text-slate-700 uppercase w-32">Kode SKU</th>
                            <th className="px-4 text-[10px] font-bold text-slate-700 uppercase">Nama Barang</th>
                    ...
                    <tbody className="divide-y divide-mist-200">
                        {inventoryData.map((item, index) => (
                            <tr key={item.id} className={`h-8 hover:bg-mist-100 group transition-colors ${!item.isActive ? 'bg-mist-50 opacity-60' : 'bg-white'}`}>
                                            ...
                                            item.conversions.map((conv, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} className="flex items-center">
                                                        <span className="text-[11px] font-bold text-slate-700">{formattedValue}</span>
                                                        <span className="text-[9px] font-semibold text-slate-400 ml-1 uppercase">{conv.name}</span>
            ...
            {isModalOpen && (
                <ModalPortal>
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/60 p-4">
                        <div className="bg-white w-full max-w-xl rounded-xl shadow-2xl border border-mist-300 overflow-hidden flex flex-col max-h-[90vh]">
                            <div className="px-5 py-4 border-b border-mist-200 flex justify-between items-center bg-mist-50">
                                <div>
                            ...
                            <form onSubmit={handleSaveItem} className="flex-1 overflow-y-auto p-6 space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-5">
                                    <div className="space-y-1.5">
                                        ...
                                        <input required type="text" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value.toUpperCase() })}
                                            className="w-full px-3 py-2 bg-white border border-mist-300 rounded-lg text-xs font-mono font-bold uppercase outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all"
                                            placeholder="CTH: BRG-001" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase">Kategori</label>
                                        <input type="text" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}
                                            className="w-full px-3 py-2 bg-white border border-mist-300 rounded-lg text-xs outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all"
                                            placeholder="Umum" />
                                    </div>
                                    <div className="col-span-2 space-y-1.5">
                                        ...
                                        <input required type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })}
                                            className="w-full px-3 py-2 bg-white border border-mist-300 rounded-lg text-xs font-semibold outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all"
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <input required type="text" value={formData.baseUnit} onChange={e => setFormData({ ...formData, baseUnit: e.target.value.toUpperCase() })}
                                            className="w-full px-3 py-2 bg-white border border-mist-300 rounded-lg text-xs font-bold text-center uppercase outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all"
                                            placeholder="PCS" />
                                    </div>
                                    <div className="space-y-1.5">
                                        ...
                                        <input type="number" value={formData.minStock} onChange={e => setFormData({ ...formData, minStock: Number(e.target.value) })}
                                            className="w-full px-3 py-2 bg-white border border-mist-300 rounded-lg text-xs font-bold outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all" />
                                    </div>
                                    <div className="col-span-2 pt-2">
                                        <label className="flex items-center gap-3 p-3 border border-mist-300 rounded-lg bg-mist-50 cursor-pointer hover:bg-white transition-colors">
                                ...
                                <div className="border-t border-mist-200 pt-5">
                                    ...
                                    {(!formData.conversions || formData.conversions.length === 0) && (
                                        <div className="text-center py-6 bg-mist-50 border border-dashed border-mist-300 rounded-lg text-slate-400 text-xs italic">
                                    ...
                                    <div className="space-y-3">
                                        {formData.conversions?.map((conv, idx) => (
                                            <div key={idx} className="flex items-center gap-2 bg-mist-50 p-2 rounded-lg border border-mist-200">
                                                <div className="w-8 h-8 flex items-center justify-center bg-white rounded border border-mist-300 text-slate-400 text-[10px] font-bold">1</div>
                                                <input
                                                    type="text"
                                                    placeholder="NAMASATUAN"
                                                    value={conv.name}
                                                    onChange={e => updateConversion(idx, 'name', e.target.value.toUpperCase())}
                                                    className="w-20 px-2 py-1.5 bg-white border border-mist-300 rounded text-xs font-bold uppercase text-center outline-none focus:border-blue-400"
                                                />
                                                ...
                                                <input
                                                    ...
                                                    className="w-20 px-2 py-1.5 bg-white border border-mist-300 rounded text-xs font-mono font-bold text-right outline-none focus:border-blue-400"
                                                />
                                                <select
                                                    value={conv.operator}
                                                    onChange={e => updateConversion(idx, 'operator', e.target.value)}
                                                    className="w-12 px-1 py-1.5 bg-white border border-mist-300 rounded text-xs font-bold text-center outline-none"
                                                >
                            ...
                            <div className="px-5 py-4 bg-mist-50 border-t border-mist-300 flex justify-end gap-3">
