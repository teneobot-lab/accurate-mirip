...
export const ReportsView: React.FC<Props> = ({ onEditTransaction }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-gradient-to-br from-cutty to-daintree p-4 gap-4 overflow-hidden transition-colors font-sans">
            {/* Header & Filter Bar */}
            <div className="bg-white/95 dark:bg-gable/90 backdrop-blur-md p-4 rounded-2xl shadow-xl border border-white/20 dark:border-spectra flex flex-wrap gap-4 items-end">
                <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-500 dark:text-cutty uppercase tracking-widest ml-1">Rentang Tanggal</label>
                ...
                <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-500 dark:text-cutty uppercase tracking-widest ml-1">Gudang</label>
                ...
                <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-500 dark:text-cutty uppercase tracking-widest ml-1">Jenis Transaksi</label>
                ...
                <div className="flex-1 flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-500 dark:text-cutty uppercase tracking-widest ml-1">Cari No. Bukti / Barang</label>
                ...
                <div className="flex gap-2">
                    <button onClick={refreshData} className="p-2.5 bg-slate-100 dark:bg-daintree rounded-xl hover:bg-slate-200 dark:hover:bg-spectra transition-colors shadow-sm text-cutty dark:text-slate-300 border border-transparent dark:border-spectra"><RefreshCw size={18} className={isLoading ? 'animate-spin' : ''}/></button>
            ...
            {/* Transaction Grid */}
            <div className="flex-1 bg-white dark:bg-gable rounded-2xl shadow-2xl border border-white/20 dark:border-spectra overflow-hidden flex flex-col">
                <div className="overflow-auto flex-1 scrollbar-thin">
                    <table className="w-full text-left border-separate border-spacing-0">
                        <thead className="bg-gable text-[10px] font-bold text-white uppercase tracking-widest sticky top-0 z-10 shadow-md">
                            <tr>
                        ...
                        <tbody className="text-[11px] divide-y divide-slate-100 dark:divide-spectra/30 text-daintree dark:text-slate-200">
                            {isLoading ? (
                                <tr>
                                ...
                                <React.Fragment key={tx.id}>
                                    <tr className={`group hover:bg-slate-50 dark:hover:bg-spectra/20 transition-all ${expandedTx.has(tx.id) ? 'bg-slate-50 dark:bg-spectra/10' : ''}`}>
                                        <td className="p-4 text-center">
                                            <button onClick={() => toggleExpand(tx.id)} className="p-1.5 hover:bg-white dark:hover:bg-daintree rounded-lg transition-colors text-slate-400 dark:text-slate-500">
                                                {expandedTx.has(tx.id) ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}
                                            </button>
                                        </td>
                                        <td className="p-4 font-mono font-bold text-cutty dark:text-slate-400">{tx.date}</td>
                                        <td className="p-4 font-black text-daintree dark:text-slate-200">
                                            <div className="flex items-center gap-2"><Hash size={12} className="text-slate-300 dark:text-spectra"/> {tx.referenceNo}</div>
                                        </td>
                                        <td className="p-4">
                                            <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter ${
                                                tx.type === 'IN' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400' : 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400'
                                            }`}>{tx.type === 'IN' ? 'Barang Masuk' : 'Barang Keluar'}</span>
                                        </td>
                                        <td className="p-4">
                                            <div className="flex flex-col">
                                                <div className="font-bold flex items-center gap-1.5 text-slate-700 dark:text-slate-300"><Box size={12} className="text-spectra"/> {warehouses.find(w => w.id === tx.sourceWarehouseId)?.name || 'Default WH'}</div>
                                        ...
                                        <td className="p-4 text-center">
                                            <div className="flex justify-center gap-2.5">
                                                ...
                                                <button 
                                                    ...
                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-600 text-blue-600 hover:text-white dark:bg-spectra/20 dark:text-white dark:hover:bg-spectra border border-blue-100 dark:border-spectra/50 rounded-xl transition-all shadow-sm active:scale-90"
                                                ...
                                                <button 
                                                    ...
                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-600 text-red-600 hover:text-white dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900 border border-red-100 dark:border-red-900/50 rounded-xl transition-all shadow-sm active:scale-90"
                                    ...
                                    {expandedTx.has(tx.id) && (
                                        <tr className="bg-slate-50/50 dark:bg-daintree/40 animate-in fade-in slide-in-from-top-1">
                                            <td colSpan={7} className="p-0">
                                                <div className="px-16 py-6 space-y-3">
                                                    <div className="grid grid-cols-12 gap-4 text-[9px] font-black uppercase text-slate-400 dark:text-cutty tracking-wider mb-2 border-b border-slate-200 dark:border-spectra pb-2">
                                                    ...
                                                    {tx.items.map((it, idx) => (
                                                        <div key={idx} className="grid grid-cols-12 gap-4 text-[10px] items-center py-1.5 hover:bg-white dark:hover:bg-gable rounded-lg px-2 -mx-2 transition-colors">
                                                            <div className="col-span-1 text-slate-300 dark:text-slate-600 font-mono">{idx + 1}</div>
                                                            <div className="col-span-2 font-mono text-spectra dark:text-white font-bold">{it.code}</div>
                                                            ...
                                                            <div className="col-span-2 text-right font-black text-spectra dark:text-cutty">
                                                                {(it.qty * (it.ratio || 1)).toLocaleString()}
                                                            </div>
                                                        </div>
                                                    ))}
                                                    {tx.notes && (
                                                        <div className="mt-4 p-4 bg-white dark:bg-gable rounded-2xl border dark:border-spectra border-dashed italic text-[10px] text-slate-500 dark:text-slate-400 shadow-inner">
                                                            <div className="font-black not-italic uppercase mb-1 text-slate-400 dark:text-cutty tracking-widest text-[8px]">Memo Internal:</div>
                                                            {tx.notes}
                                                        </div>
                                                    )}
                                                </div>
            ...
            <style>{`
                .report-input { 
                    @apply border border-cutty/20 dark:border-spectra bg-slate-50 dark:bg-daintree text-daintree dark:text-white rounded-xl px-4 py-2.5 text-[11px] font-bold outline-none focus:ring-2 focus:ring-spectra focus:border-spectra transition-all shadow-sm placeholder:text-slate-400; 
                }
            `}</style>
        </div>
    );
