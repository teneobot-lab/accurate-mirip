        ...
        <div className="flex flex-col h-full bg-white font-sans animate-in fade-in duration-300">
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="w-72 border-r border-mist-300 flex flex-col shrink-0 bg-mist-50/30">
                    <div className="px-4 py-2 bg-mist-100/50 border-b border-mist-300 flex items-center justify-between">
                            ...
                            Array.from({ length: 5 }).map((_, i) => (
                                <div key={i} className="p-3 border-b border-mist-200">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="h-3 w-32 bg-mist-200 rounded animate-pulse"></div>
                                        <div className="h-2 w-12 bg-mist-200 rounded animate-pulse"></div>
                                    </div>
                                    <div className="flex justify-between items-end mb-2">
                                        <div className="h-4 w-16 bg-mist-200 rounded animate-pulse"></div>
                                        <div className="h-3 w-10 bg-mist-200 rounded animate-pulse"></div>
                                    </div>
                                    <div className="h-1 w-full bg-mist-100 rounded-full overflow-hidden"></div>
                                </div>
                            ))
                        ) : (
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    <div className="px-4 py-2 bg-mist-100/50 border-b border-mist-300 flex items-center justify-between shrink-0">
                        ...
                        <button className="text-[10px] font-semibold text-brand hover:underline">Lihat Semua Laporan</button>
                    </div>
                        ...
                        <table className="w-full border-collapse text-left table-fixed">
                            <thead className="bg-white sticky top-0 z-10 border-b border-mist-300 shadow-[0_1px_0_rgba(0,0,0,0.05)]">
                                <tr className="h-8">
                                    ...
                                    <th className="px-4 text-[10px] font-bold text-slate-500 uppercase w-20 text-center">Items</th>
                                ...
                                {isLoading ? (
                                    Array.from({ length: 10 }).map((_, i) => (
                                        <tr key={i} className="h-8">
                                            <td className="px-4"><div className="h-3 w-20 bg-mist-200 rounded animate-pulse"></div></td>
                                            <td className="px-4"><div className="h-3 w-16 bg-mist-200 rounded animate-pulse"></div></td>
                                            <td className="px-4 text-center"><div className="h-3 w-8 bg-mist-200 rounded animate-pulse mx-auto"></div></td>
                                            <td className="px-4"><div className="h-3 w-48 bg-mist-200 rounded animate-pulse"></div></td>
                                            <td className="px-4 text-center"><div className="h-3 w-4 bg-mist-200 rounded animate-pulse mx-auto"></div></td>
                                            <td className="px-4"><div className="h-3 w-16 bg-mist-200 rounded animate-pulse"></div></td>
                                        </tr>
                                    ))
                                ) : (
                                    data.recentTx.map(tx => (
                                        <tr key={tx.id} className="h-8 hover:bg-mist-50/50 group transition-colors">
                                            ...
                                            <td className="px-4 text-[11px] text-slate-700 truncate font-medium">
                                                {tx.partnerName ? <span className="uppercase text-[10px] bg-mist-200 px-1 rounded mr-2 text-slate-600">{tx.partnerName}</span> : null}
                ...
                <div className="w-64 border-l border-mist-300 flex flex-col shrink-0 bg-mist-50/50">
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <div className="bg-brand p-4 rounded-xl text-white shadow-lg relative overflow-hidden">
                            ...
                            <p className="text-[10px] opacity-90 leading-relaxed">Jangan lupa cek stok opname akhir bulan ini untuk area Warehouse A.</p>
                        </div>
                    </div>
                </div>
            </div>
