...
export const StockCardView: React.FC<Props> = ({ item, onBack }) => {
  ...
  const StatCard = ({ label, val, color }: any) => (
      <div className="flex flex-col px-4 border-r border-mist-300 last:border-0">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col font-sans animate-in slide-in-from-bottom-2 duration-300">
        ...
        {/* 1. HEADER BAR */}
        <div className="h-14 border-b border-mist-300 flex justify-between items-center px-4 bg-white shrink-0">
            <div className="flex items-center gap-4">
                <button onClick={onBack} className="p-2 hover:bg-mist-100 rounded-full text-slate-500 transition-colors">
                    <ArrowLeft size={18} />
                </button>
                <div>
                    <div className="flex items-center gap-2">
                        <h1 className="text-sm font-bold text-slate-800">{item.name}</h1>
                        <span className="text-[10px] bg-mist-100 text-slate-500 px-1.5 py-0.5 rounded font-mono font-semibold">{item.code}</span>
                    </div>
            ...
            <div className="flex items-center gap-2">
                <div className="hidden md:flex items-center gap-2 bg-mist-50 border border-mist-300 rounded-lg p-1 mr-4">
                     ...
                     <div className="w-px h-4 bg-mist-300"></div>
                ...
                <button onClick={() => window.print()} className="p-2 text-slate-400 hover:text-slate-800 hover:bg-mist-100 rounded-lg transition-colors"><Printer size={18}/></button>
            </div>
        </div>

        {/* 2. SUMMARY STRIP */}
        <div className="bg-mist-50 border-b border-mist-300 py-3 flex items-center px-4 shrink-0 overflow-x-auto">
             ...
             <div className="flex flex-col px-4 border-l-2 border-mist-300 bg-white ml-2 rounded-r-lg shadow-sm">
        ...
        <div className="flex-1 overflow-auto bg-white relative">
            <table className="w-full text-left border-collapse table-fixed min-w-[900px]">
                {/* SILVER MIST HEADER */}
                <thead className="bg-mist-300 sticky top-0 z-10 border-b border-mist-300 shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
                    <tr className="h-9">
                        <th className="px-3 w-28 text-[10px] font-bold text-slate-700 uppercase">Tanggal</th>
                        <th className="px-3 w-36 text-[10px] font-bold text-slate-700 uppercase">No. Referensi</th>
                        <th className="px-3 w-40 text-[10px] font-bold text-slate-700 uppercase">Gudang</th>
                        <th className="px-3 w-20 text-[10px] font-bold text-slate-700 uppercase text-center">Tipe</th>
                        <th className="px-3 text-[10px] font-bold text-slate-700 uppercase">Keterangan / Partner</th>
                        <th className="px-3 w-24 text-[10px] font-bold text-slate-700 uppercase text-right bg-emerald-50/30 text-emerald-600">Masuk</th>
                        <th className="px-3 w-24 text-[10px] font-bold text-slate-700 uppercase text-right bg-rose-50/30 text-rose-600">Keluar</th>
                        <th className="px-3 w-28 text-[10px] font-bold text-slate-700 uppercase text-right">Saldo</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-mist-100">
                     {/* Opening Balance Row */}
                     <tr className="bg-mist-50 h-8">
                     ...
                     {ledgerRows.map((row) => (
                         <tr key={row.id} className="h-8 hover:bg-mist-50 group transition-colors cursor-default">
                             ...
                             <td className="px-3 text-right font-mono text-[11px] font-bold text-slate-700 bg-mist-50/50">
