        ...
        <div className="overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg">
                    <div className="text-xs font-bold text-slate-500 uppercase mb-1">Total Stock</div>
                    <div className={`text-3xl font-mono font-bold ${stockData.total <= item.minStock ? 'text-red-600' : 'text-slate-800'}`}>
                        {stockData.total} <span className="text-sm font-sans font-normal text-slate-500">{item.baseUnit}</span>
                    </div>
                 ...
                 <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg">
                    <div className="text-xs font-bold text-slate-500 uppercase mb-2">Conversions</div>
                    <div className="space-y-1">
                        {item.conversions.length > 0 ? item.conversions.map((c, i) => (
                            <div key={i} className="text-sm text-slate-700 flex justify-between border-b border-dashed border-slate-300 last:border-0 py-1">
                                <span>1 {c.name}</span>
                                <span className="font-mono font-bold text-slate-900">= {c.ratio} {item.baseUnit}</span>
                            </div>
            ...
            <div>
                ...
                <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-100 text-xs text-slate-500 uppercase font-bold">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {stockData.breakdown.map(wh => (
                                <tr key={wh.name}>
                                    <td className="p-3 font-medium text-slate-700">{wh.name}</td>
                                    <td className="p-3 text-right font-mono font-bold">{wh.qty}</td>
                                    <td className="p-3">
                                        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                            <div 
                                                className={`h-full ${wh.qty > 0 ? 'bg-blue-500' : 'bg-slate-300'}`} 
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {history.length === 0 ? (
                                ...
                                <tr key={idx} className="hover:bg-slate-50">
                                    ...
                                    <td className="p-2 font-mono text-slate-600">{h.ref}</td>
                                    <td className="p-2">
                                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                                            h.type === 'IN' || h.type === 'ADJUSTMENT' ? 'bg-emerald-100 text-emerald-700' :
                                            h.type === 'OUT' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
                                        }`}>{h.type}</span>
                                    </td>
                                    <td className={`p-2 text-right font-bold ${h.type === 'OUT' || h.type === 'TRANSFER' ? 'text-red-600' : 'text-emerald-600'}`}>
                                        {h.qty}
                                    </td>
                                    <td className="p-2 text-slate-500">{h.unit}</td>
                                    <td className="p-2 text-slate-500 truncate max-w-[100px]">{h.wh}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
