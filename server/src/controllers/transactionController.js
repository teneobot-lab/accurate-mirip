...
exports.createTransaction = async (req, res, next) => {
    try {
        ...
        if (!referenceNo || referenceNo.trim() === "") return res.status(400).json({ success: false, message: 'Nomor referensi wajib diisi' });
        if (!sourceWarehouseId) return res.status(400).json({ success: false, message: 'Gudang asal wajib dipilih' });
        if (!items || !Array.isArray(items) || items.length === 0) return res.status(400).json({ success: false, message: 'Baris item transaksi tidak boleh kosong' });

        let result;
        if (type === 'IN') result = await inventoryService.processInboundTransaction(req.body, user);
        else if (type === 'OUT') result = await inventoryService.processOutboundTransaction(req.body, user);
        else return res.status(400).json({ success: false, message: 'Tipe transaksi tidak valid' });
...
exports.updateTransaction = async (req, res, next) => {
    try {
        ...
        if (!req.body.items || !Array.isArray(req.body.items) || req.body.items.length === 0) return res.status(400).json({ success: false, message: 'Item transaksi tidak boleh kosong' });

        const result = await inventoryService.updateTransaction(id, req.body, user);
        return res.json({ success: true, ...result });
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: 'Nomor referensi sudah digunakan' });
        if (error.code === 'INSUFFICIENT_STOCK') return res.status(409).json({ success: false, message: `Update Gagal: ${error.message}` });
        next(error);
    }
};

/**
 * DELETE TRANSACTION (Anti-502 Fix)
 */
exports.deleteTransaction = async (req, res, next) => {
    const { id } = req.params;
    console.log(`[TX_CONTROLLER] ATTEMPT DELETE: ${id}`);
    
    // Explicitly close connection to prevent Proxy/Nginx waiting for keep-alive
    ...
    try {
        ...
        if (error.code === 'INSUFFICIENT_STOCK') {
            return res.status(409).json({ success: false, message: `Hapus Gagal: Stok tidak cukup untuk revert. ${error.message}` });
        }
        next(error);
    }
};
