...
function App() {
  ...
  const handleLogin = useCallback((user: any) => {
      ...
      setCurrentUser(user);
      setIsLoggedIn(true);
      // Reset tab ke dashboard setiap login baru
    ...
    <ToastProvider>
      {isLoadingSession ? (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4">
          ...
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Memulai GudangPro...</div>
        </div>
      ) : !isLoggedIn ? (
        ...
        <div className="flex h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
          ...
          <main className="flex-1 flex flex-col overflow-hidden relative">
             <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 z-20 transition-colors shadow-sm">
                 <div className="flex items-center gap-6">
                    ...
                    <div className="flex-shrink-0">
                        <h2 className="text-base font-bold text-slate-800 dark:text-slate-100 leading-none">
                            ...
                            {activeTab === 'INVENTORY' && 'Inventory Stok'}
                            {activeTab === 'REPORTS' && 'Mutasi Barang'}
                            {activeTab === 'SETTINGS' && 'Konfigurasi'}
                            {activeTab === 'REJECT' && 'Barang Reject'}
                        </h2>
                    </div>
                 </div>
             ...
             {showTransactionModal && (
                 <TransactionForm 
                    ...
                    onSuccess={() => {
                        setShowTransactionModal(null);
                        setEditingTransaction(null);
                        // Force refresh by toggling active tab briefly
